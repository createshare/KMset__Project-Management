...
## GitLab Git 仓库+分支管理+代码管理

GitLab的仓库用于存放当前项目的所有文件，包括项目成员所有的产出文件、提交记录、项目的分支、标签和项目的概要信息。点击菜单栏中的“仓库”，可以展开仓库。

### 克隆仓库

- 仓库的数据可以克隆到项目组成员的本地仓库中，对于组成员来说，GitLab的仓库可以看做为远程仓库，通过上文对GIT“创建仓库”的介绍，可以很容易地将GitLab的仓库克隆到本地。
- 需要注意的是，在多人团队开发中，为了保证本地仓库与远程仓库尽量同步，必须适时提交和更新数据，这样做可以减小未来合并分支时，由于修改冲突而导致的合并失败的概率。

### 提交记录

- 在“仓库”展开栏中，可以看到“提交”项，该项中记录了所有成员的提交记录。

- 提交记录是项目开发中，过程完整性的一部分，通过提交记录可以方便地看到该成员在指定提交节点前所有的修改记录，方便追溯开发过程中的一些问题。

  ![GitLab 查看项目提交记录](figures/GitLab 查看项目提交记录.jpg)
...
- 分支管理主要是通过Git配置管理工具来实现。
  ...
  | tag 标签            | 每次发布或建立基线时，必须从 master 分支上，建一个Tag；<br />Tag 命名：以软件版本号或基线版本号命名。<br />在“标签”项中，可以给当前版本打一个标签，标签没有实质性的作用，是一种版本管控的手段，旨在告诉全员被标记的版本处于何种状态，状态具体的含义可以查看标签创建时的备注信息。 |
...
- 项目成员在将本地仓库推送到 GitLab 仓库后，需要在 GitLab 上发出一个合并请求  MR（Merge Request），用于通知项目管理员将该成员的分支合并到主分支master上。
- 项目管理员收到请求后，会 Review 推送上来的数据，确认无误后才会合并到主分支上，形成作业闭环。
- Review：合并代码和提交日志查看
- 如果项目管理员发现推送上来的数据有问题，则会把有问题的地方标记出来（添加评论），请求发起人则需要对有问题的地方进行确认和修改，并回复项目管理员。
- 经过反复讨论并确认无误后，项目管理员才会将分支合并到主分支上。

![GitLab 创建合并请求](figures/GitLab 创建合并请求.jpg)
...
额外补充一点，开发分支所提交上来的代码可以在 Pipelines 构建中查看是那个用户提交上来的，默认在每个分支都会进行编译，所以都能查到是谁提交过来的代码。



# GitLab 的 issue 议题

- GitLab的议题用于跟踪项目中需要改进或解决的问题，问题可以是 Bug、任务或想法。
- 通常在项目开发过程中，一些流程性的问题通过 GitLab 的议题功能管理比较好。
- 它能很方便地定位问题的流程状态，有指导性地“引领”团队成员接下来的工作。
- 项目中所有已创建的议题都可以在议题列表中找到，同时这里也是议题创建的地方。
- 议题列表能够宏观地展示出当前项目所有未关闭和已关闭的议题，可以使用 GitLab 提供的搜索功能，按照不同维度（作者、指派人、里程碑、标记...）检索议题。

## 新建议题           

![GitLab 新建议题：Issue 或 Bug](figures/GitLab 新建议题：Issue 或 Bug.jpg)



## 议题标记

- 议题的标记表示议题当前的状态，它是议题的过程表示量，一组通用的议题标记如（pre-status、in-progress、completed）。
- 在议题的“标记”项中，可以创建一个议题标记，点击右上角的“新建标记”按钮，创建一个标记。

![GitLab 创建议题标记](figures/GitLab 创建议题标记.png)



## 议题看板

- 议题列表的最大问题就是它无法很直观地展示出所有议题的状态信息，不同状态的议题都混杂在一起，没有可读性。
- 而议题看板恰恰弥补了这方面的不足，通过看板图可以很直观地定位议题当前所处的状态在整个流程中的位置。
- 在议题看板中可以添加“板块列表”，用于展示议题的更多状态。板块列表的创建可以来源于议题标记。
- 当议题的状态发生改变时，可以在看板中直接将议题拖动到对应的板块列表中完成状态切换。

![GitLab 议题看板](figures/GitLab 议题看板.jpg)
...
- Wiki 的作用是用于保存项目信息和关键知识点，是项目文档的管理库。
...
- CI/CD是一个内置在GitLab中的工具，用于通过持续方法进行软件开发。持续方法包括“持续集成”与“持续交付和部署”。
...
- 持续集成
  - 持续集成的工作原理是将小的代码块推送到GIT仓库托管的应用程序代码库中，并且每次推送时，都要运行一系列脚本来构建、测试和验证代码更改，然后再将其合并到主分支中。
...
- 持续交付
  ...
  - 相当于更进一步的CI，可以在每次推送到仓库默认分支的同时将应用程序部署到生产环境。
- 持续部署
  - 与持续交付类似，但不同之处在于，你无需将其手动部署，而是将其设置为自动部署。
  - 完全不需要人工干预即可部署你的应用程序。
